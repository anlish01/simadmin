...
- VoWiFi 数据通道新增 SOCKS5 UDP Associate 前置代理，支持可选账号密码认证、配置热更新、自动重连与受控直连回退。
...
- 优化 VoWiFi 代理候选切换与直连回退顺序，并在备份恢复代理配置后同步刷新运行时连接。

#### 🐞 Bug 修复

- 当前网络不支持 IPv6 时，跳过 IPv6 连通性检测、重试及失败通知，避免系统启动后产生无效告警。
- 修正磁盘空间充足时仍在启动后告警的问题：低空间需同时满足剩余比例与容量阈值，任一指标恢复即可解除告警。
